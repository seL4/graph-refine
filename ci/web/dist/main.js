...
function github_run(title, {repo, run}) {
  return `
    <div class="w-40">
      <div class="text-xs text-slate-400">${title}</div>
      <div class="font-mono">
        <a href="https://github.com/${esc_html(repo)}/actions/runs/${esc_html(run)}" class="hover:underline">${esc_html(run)}</a>
      </div>
    </div>
  `;
}
    ...
    <div class="grid grid-cols-1 divide-y divide-slate-600 gap-y-1 -mx-2 my-3 px-2 py-2 rounded-md bg-slate-800 hover:ring-1 hover:ring-slate-400">
      <div class="flex flex-wrap gap-x-8 gap-y-1">
        <div class="w-40">
          ...
          <div class="font-mono">${esc_html(job_id.substring(0,10))}</div>
        </div>
        <div class="flex flex-wrap gap-x-8 gap-y-1">
          <div class="flex flex-wrap gap-x-8 gap-y-1">
...
async function render_target(job_id, target) {
  ...
  Object.entries(functions_status).forEach(([name, info]) => {
    ...
    function fun_name_cell({indicator_colour}) {
      ...
      return indicator + esc_html(name);
    }

    if (started === undefined) {
      waiting_functions.push({name, content: `
        <tr class="hover:text-semibold">
          <td class="px-2 py-1">${fun_name_cell({indicator_colour: "bg-gray-500"})}</td>
        </tr>
      `});
      return;
    }
    ...
    function report_log_cell() {
      const fun_url = `jobs/${esc_html(job_id)}/targets/${esc_html(target)}/functions/${esc_html(name)}/${esc_html(run_id)}`;
  ...
  document.getElementById("content-container").innerHTML = `
    <section class="mt-6">
      ...
      <div class="grid grid-cols-1 gap-y-3 -mx-2 mt-3 mb-8 px-2 py-2 rounded-md bg-slate-800">
        ...
        <div class="flex flex-wrap gap-x-8 gap-y-3">
          ${!job_info?.github?.proof ? '' : github_run("proof run", job_info.github.proof)}
          ${!job_info?.github?.decompile ? '' : github_run("decompile run", job_info.github.decompile)}
          <div class="w-40">
            <div class="text-xs text-slate-400">seL4 commit</div>
            <div class="font-mono">${esc_html(versions["seL4"].substring(0,10))}</div>
          </div>
          <div class="w-40">
            <div class="text-xs text-slate-400">l4v commit</div>
            <div class="font-mono">${esc_html(versions["l4v"].substring(0,10))}</div>
          </div>
        </div>
      </div>
